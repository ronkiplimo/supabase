...
Supabase tracks which migrations have been applied on each database in a table called `supabase_migrations.schema_migrations`. When you run `supabase db push`, it compares your local `supabase/migrations` folder against that table and runs only the unapplied ones, in order.
...
If someone used the Dashboard's SQL editor or Table Editor on the remote database, the schema has changed but there's no migration file for it. Pull the current remote state into a migration file to get back in sync:
