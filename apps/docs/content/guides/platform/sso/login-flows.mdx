...
When configuring SSO for your organization, you can choose between two different login flows: **IdP-initiated** and **SP-initiated**. Understanding the difference helps you provide the best experience for your users.

<Admonition type="tip">

**Quick decision guide:** Most enterprises use IdP-initiated flow for its simplicity and better user experience. Enable SP-initiated only if you need users to start their login journey at supabase.com.

See our [Choosing the Right Login Flow](/docs/guides/platform/sso/choosing-login-flow) guide for use case examples.
...
**That's it!** Users can now access Supabase through your IdP's app catalog.

<Admonition type="note">

**Domain configuration optional:** With IdP-initiated flow, you don't need to configure email domains. Your identity provider handles all authentication routing.
...
**Domain restrictions apply:** Only users with email addresses matching your configured domains can use SP-initiated login. Users with other domains cannot sign in via SSO at supabase.com (but can still use IdP-initiated flow if you configure it in your IdP).
...
Users simply click the appropriate tile for the environment they need.
...
They'll receive an error message indicating no SSO provider found for their email domain. They can still sign in using password or social auth (if they have a non-SSO account).

### Can I disable SP-initiated flow after enabling it?
