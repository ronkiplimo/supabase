...
Many enterprises need multiple SSO providers configured within Supabase to support different environments, teams, or organizational structures. This guide explains when and how to set up multiple providers effectively.
...
**Solution:** Use IDP-initiated flow, which doesn't require domain configuration. You can create unlimited SAML apps under the same domain.

<Admonition type="tip">

**Recommended pattern:** Configure each environment as IDP-initiated only (no domains). Users access each environment through different app tiles in your identity provider.

For technical details, see [Understanding SSO Login Flows](/docs/guides/platform/sso/login-flows#multiple-saml-apps-per-domain).
...
1. Navigate to [SSO settings](/dashboard/org/_/sso)
2. Enable "Single Sign-On"
3. **Leave "Enable SP-initiated login" OFF** (critical)
...
| Environment | Organization  | IDP App Name       | ACS URL                              | Users Assigned    |
| ----------- | ------------- | ------------------ | ------------------------------------ | ----------------- |
| Dev         | `dev-org`     | "Supabase Dev"     | `https://...dev-org.../saml/acs`     | All engineers     |
| Staging     | `staging-org` | "Supabase Staging" | `https://...staging-org.../saml/acs` | QA + Release mgrs |
| Production  | `prod-org`    | "Supabase Prod"    | `https://...prod-org.../saml/acs`    | Production users  |
...
- Both orgs add `company.com` as a domain
...
**SP-initiated routing with multiple providers:** When multiple organizations use SP-initiated with the same domain, the first provider where the user is a member will be used. This can cause confusion. **IDP-initiated is recommended** for clarity.
...
See [SSO Testing and Best Practices](/docs/guides/platform/sso/testing-best-practices) for comprehensive testing procedures.
...
Users will only see app tiles they're assigned to.
...
**Symptom:** User clicks "Supabase Prod" tile but sees the dev environment.
...
**Symptom:** SSO works in dev but fails in staging or production.
...
**Symptom:** User cannot find "Supabase Staging" tile in IDP dashboard.
...
4. Verify user is checking correct IDP portal (some orgs have multiple)

### Auto-join adding users to wrong organization

**Symptom:** User joins dev environment when they should join production.

**Cause:** User clicked wrong app tile, auto-join is enabled.
...
**Symptom:** With SP-initiated enabled and same domain in multiple orgs, users get routed to unexpected organization.

**Cause:** SP-initiated routing uses first matching provider.
