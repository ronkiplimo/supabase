...
) fel ON content.execution_id = fel.execution_id
WHERE 
  fel.status_code = 500
    AND 
  content.level = 'error'
    AND
    (
      content.event_type = 'Log'
        OR
      content.event_type = 'UncaughtException'
    )
    AND
  fl.event_message LIKE '%Error:%file:///%'
ORDER BY function_name, timestamp
LIMIT 50;
```
...
The log will contain an `event_message` that should showcase the error type. It may look like:
...
It's common for developers to make requests to external resources from an edge function. If, for some reason, the object returns as `null` or `undefined`, and you do not type check, then you're likely to encounter a `TypeError`
...
A `ReferenceError` occurs when one tries to reference a variable that does not exist in the scope it was referenced in. Often times caused by a typo or missing import.
...
A `SyntaxError` error occurs Deno's grammatical rules are violated. For instance, failing to close parenthesis will cause the error to emerge:
```js
console.log('unclosed' ; // Uncaught SyntaxError: missing ) after argument list
```

In most cases, syntax violations can be fixed by fixing a typo. There is a special case that is common enough that it is worth providing an example over: [CORS](https://developer.mozilla.org/en-US/docs/Glossary/Preflight_request) violations.
...
So, when encountering CORS errors, it is still important to check the logs or run the request outside the browser to make sure it is actually the primary factor and not a side-effect of a larger issue.
